...
export const createNotificationSchema = z.object({
  tenant_id: z
    ...
    .refine((v) => v === undefined || (!Number.isNaN(v) && v > 0), "Tenant ID tidak valid"),
  channel: z.enum(["IN_APP", "EMAIL", "PUSH"]).default("IN_APP"),
  type: z.enum(["SYSTEM", "BILLING", "RAT", "LOAN", "SAVINGS", "CUSTOM"]).default("SYSTEM"),
  category: z.string().min(1, "Kategori wajib diisi"),
  target_type: z.enum(["SINGLE", "ALL", "GROUP"]).default("ALL"),
  title: z
    .string({ errorMap: () => ({ message: "Judul wajib diisi" }) })
    .min(2, "Judul minimal 2 karakter"),
  body: z
    .string({ errorMap: () => ({ message: "Isi pesan wajib diisi" }) })
