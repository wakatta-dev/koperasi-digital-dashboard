...
import { depositSavings } from "@/services/api";
...
export function SavingsDepositDialog({ memberId, onSuccess }: Props) {
  ...
  async function onSubmit(values: FormValues) {
    try {
      const targetMemberId = memberId
        ? Number(memberId)
        : Number(values.member_id);
      if (!targetMemberId) throw new Error("member_id kosong");
      await depositSavings(targetMemberId, {
        type: values.type,
      ...
      toast.error(e?.message || "Gagal membuat setoran");
    }
  }
    ...
    <Dialog open={open} onOpenChange={setOpen}>
      ...
      <DialogContent>
        <DialogHeader>
          ...
          <DialogDescription>Catat setoran untuk anggota.</DialogDescription>
        </DialogHeader>
        <Form {...form}>
            ...
            <FormField
              ...
              render={({ field }) => (
                <FormItem>
                  ...
                  <FormControl>
                    <Input type="number" placeholder="2000" {...field} value={field.value as any} />
                  </FormControl>
                  <FormMessage />
                </FormItem>
              )}
