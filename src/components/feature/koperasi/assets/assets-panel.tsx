...
import type { Asset, AssetRequest } from "@/types/api";
...
type AssetFormState = AssetRequest;
      ...
      <Card>
        ...
        <CardContent>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            ...
            <Input placeholder="Kategori" value={form.category} onChange={(e) => setForm((s) => ({ ...s, category: e.target.value }))} />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-4 gap-3 mt-3">
            <DatePicker
              placeholder="Tgl Akuisisi"
              value={form.acquisition_date || undefined}
              onChange={(v) => setForm((s) => ({ ...s, acquisition_date: v || "" }))}
            ...
            <Input
              ...
              onChange={(e) =>
                setForm((s) => ({ ...s, acquisition_cost: Number(e.target.value || 0) }))
              }
            />
            <Input
              placeholder="Metode Depresiasi"
              value={form.depreciation_method}
              onChange={(e) => setForm((s) => ({ ...s, depreciation_method: e.target.value }))}
            />
            <Input
              type="number"
              ...
              onChange={(e) =>
                setForm((s) => ({ ...s, useful_life_months: Number(e.target.value || 0) }))
              }
            />
          </div>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mt-3">
            <Input placeholder="Lokasi" value={form.location} onChange={(e) => setForm((s) => ({ ...s, location: e.target.value }))} />
            <div />
            <Button
              onClick={async () => {
          ...
          <div className="space-y-3">
            ...
            {items.map((a) => (
              <div key={a.id} className="grid grid-cols-1 md:grid-cols-4 gap-3 p-3 border rounded-md items-center">
                ...
                <div>
                  <div className="text-sm">Biaya: {numberFormatter.format(a.acquisition_cost)}</div>
                ...
                <div className="flex items-center gap-2 justify-end">
                  ...
                  <Button
                    ...
                    size="sm"
                    onClick={async () => {
                      await updateAsset(a.id, { name: a.name });
                      await load();
                    }}
                  >
                    Edit
