...
export function LandingNavbar() {
  ...
  const links = NAV_LINKS.filter(
    (link) => !link.requiresShortcut || showShortcutLinks
  );
            ...
            {links.map((link) => {
              ...
              return (
                <Link
                  key={link.label}
                  href={link.href}
                  className="text-sm font-medium text-gray-700 dark:text-gray-300 hover:text-[#4338ca] dark:hover:text-[#4338ca] transition"
                >
                  {link.label}
                </Link>
              );
