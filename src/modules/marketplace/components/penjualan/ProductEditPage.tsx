...
export function ProductEditPage({ id }: ProductEditPageProps) {
  ...
  const { update, addImage, deleteImage, setPrimaryImage } =
    useInventoryActions();
  ...
  const variantRows = useMemo<EditableVariantRow[]>(() => {
    ...
    const groups = new Map(
      (variantsData.variant_groups ?? []).map((group) => [
        group.id,
        group.name,
      ]),
    );
  ...
  const updateVariantRow = (
    ...
    setEditableVariants((prev) =>
      prev.map((row) =>
        row.optionId === optionId ? { ...row, ...updates } : row,
      ),
    );
  };
  ...
  const handleSave = async () => {
    ...
    const parsedWeight = Number.isFinite(weightValue as number)
      ? weightValue
      : undefined;
    try {
      ...
      <div className="space-y-6 pb-12">
        ...
        <section className="surface-card p-6">
          ...
          <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 gap-4 mb-6">
            ...
            <Button
              ...
              className="aspect-square h-full rounded-lg border-2 border-dashed border-gray-300 dark:border-gray-600 flex flex-col items-center justify-center cursor-pointer hover:border-indigo-600 dark:hover:border-indigo-600 transition-colors group"
            >
          ...
          >
            ...
            <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
              Atau klik untuk memilih file dari komputer Anda (Maks. 5MB per
              file)
            </p>
          </div>
        </section>
        ...
        <ConfirmActionDialog
          ...
          onConfirm={async () => {
            ...
            setEditableVariants((prev) =>
              prev.filter(
                (row) => row.optionId !== pendingDeleteVariant.optionId,
              ),
            );
