  ...
  const columns = useMemo<ColumnDef<PaymentListItem, unknown>[]>(
    () => [
      ...
      },
      {
        ...
        cell: ({ row }) => (
          <Badge
            className={`rounded-full px-2.5 py-0.5 text-xs ${PAYMENT_STATUS_BADGE_CLASS[row.original.status]}`}
          >
            {row.original.status}
          </Badge>
        ),
    ...
    <Card className="border-gray-200 shadow-sm dark:border-gray-700">
      <CardHeader className="flex flex-row items-center justify-between border-b border-gray-200 p-5 dark:border-gray-700">
        <div>
          ...
          <p className="text-xs text-gray-500 dark:text-gray-400">
            Log of incoming customer payments
          </p>
        </div>
      ...
      <CardContent className="p-0">
        <TableShell
          columns={columns}
          data={filteredRows}
          getRowId={(row) => row.payment_number}
          emptyState="Tidak ada pembayaran."
